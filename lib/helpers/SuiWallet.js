...
const SuiWallet = {
    ...
    signAndExecuteTransactionBlock: async (tx) => {
        if(!SuiWallet.activeProvider) throw new Error(errors.NO_ACTIVE_PROVIDER_ERROR);
        if(!hasStandardFeature(features.signAndExecuteTransactionBlock, SuiWallet.activeProvider))
            throw new Error(errors.NOT_AVAILABLE_FEATURE);

        return SuiWallet.activeProvider?.features[features.signAndExecuteTransactionBlock]
